...
enum FilterOption: Identifiable, Equatable {
    ...
    var id: String {
        ...
        case .priceLowToHigh: return "low"
        case .priceHighToLow: return "high"
        }
    }
    ...
    var title: String {
        switch self {
        case .all: return "All"
        case .category(let value): return value.capitalized
...
private func bindSearchAndFilter() {
    ...
    func fetchProducts() {
        print("🟡 ViewModel: fetchProducts started")
        isLoading = true
        errorMessage = nil
        ...
        repository.fetchProducts()
            .sink { [weak self] completion in
                ...
                switch completion {
                case .finished:
                    print("✅ ViewModel: fetchProducts completion finished")
                case .failure(let error):
                    print("❌ ViewModel: fetchProducts failed with error: \(error.localizedDescription)")
                    self?.errorMessage = error.localizedDescription
                }
            } receiveValue: { [weak self] products in
                print("✅ ViewModel: received \(products.count) products")
...
private func updateFavoriteState(for productId: Int) {
    ...
    private func fetchCategories() {
        print("🟡 ViewModel: fetchCategories started")

        repository.fetchCategories()
            .sink { completion in
                switch completion {
                case .finished:
                    print("✅ ViewModel: fetchCategories finished")
                case .failure(let error):
                    print("❌ ViewModel: fetchCategories failed: \(error.localizedDescription)")
                }
            } receiveValue: { [weak self] categories in
                print("✅ ViewModel: received categories = \(categories)")
